...
class Graph:
    ...
    def remove_conn_dir(self, node1, node2):
        ...
        # Может ли пройти от node1 к node2
    ...
    def Dijkstra_algorithm(self, node):
...
graph = Graph.create_from_nodes([a, b, c, d, e, f, g])

graph.connect(a, b)
graph.connect(a, c)
graph.connect(a, e)
graph.connect(b, c)
#"Соединение графов. Может быть как с заданной длиной, так и без. Сделал неориентированный граф, ну просто первое что пришло в голову т.к неоговорено."
graph.connect(b, d, 3)
graph.connect(c, d, 12)
graph.connect(c, f, 16)
graph.connect(d, e, 15)
graph.connect(g, a, 5)
graph.connect(g, b, 8)
#Метод вывода матрицы смежности графа.
graph.print_adj_mat()
#Красивый вывод поиска пути от выбранной вершины до каждой иной
print([(weight, [n.data for n in node]) for (weight, node) in graph.Dijkstra_algorithm(f)])
